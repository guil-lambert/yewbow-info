...
export function useFetchedTokenDatas(tokenAddresses: string[]): {
  ...
  const {
    ...
    error: error48,
    data: data48,
  } = useQuery<TokenDataResponse>(TOKENS_BULK(parseInt(block48?.number), tokenAddresses), { client: dataClient })

  const {
    loading: loadingWeek,
    error: errorWeek,
    data: dataWeek,
  } = useQuery<TokenDataResponse>(TOKENS_BULK(parseInt(blockWeek?.number), tokenAddresses), { client: dataClient })
  ...
  const formatted = tokenAddresses.reduce((accum: { [address: string]: TokenData }, address) => {
    ...
    const feesUSD =
      current && oneDay
        ...
        : 0
    if (current && volumeUSD > 1000 && tvlUSD > 1000 && priceUSD > 10 ** -12) {
      accum[address] = {
        exists: !!current,
        address,
